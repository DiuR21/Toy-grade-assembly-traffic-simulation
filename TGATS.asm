...
;		   MESS DATA PART
;*********************************
MESS1 DB 0DH,0AH,"Traffic Simulation V0.5 $"
...
;		1. can pass
;		2. can't pass
;		3. red light but before the lgiht
;*********************************
CAR_S PROC NEAR
	;NTS
	PUSH CX
...
;		  CAR RUN_I PART
;		usage:
;		1.Mov Cars' State into CARS_P
;		2.Mov Cars' Running Pos into
;			CARR_P
;		3.Mov Car's Running Boundry into
;			CARR_B
;		4.Mov Car Number into CAR_N
;		5.Mov Car End Pos into CAR_E
;*********************************
CARR_I PROC NEAR
	PUSH AX
	PUSH BX
	...
	CMP [SI], AX
	JE CARRI_S
	JMP CARRI_RLB ;Red Light Before
	CARRI_RUN:
		MOV AX, [DI]
		ADD AX, 15D
		MOV [DI], AX
	...
	CARRI_RLB:
		...
		ADD AX, 10D
		CMP AX, BX
		JA CARRIRLB_S ;Red Light Before STOP
		MOV [DI], AX
		JMP CARRI_REE
		CARRIRLB_S:
		MOV [DI], BX
